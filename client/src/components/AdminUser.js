  ...
  const searchUser = async () => {
    try {
      const response = await axios.get(
        `${deploy_api_url}/api/users/id/${userId}`,
        {
  ...
  const disableUser = (disableorenable) => {
    ...
    const requestUrl =
      disableorenable === "disable"
        ? `${deploy_api_url}/api/users/disableuser/${user.email}`
        : `${deploy_api_url}/api/users/enable/${user.email}`;
    axios
      .get(requestUrl)
      .then((response) => {
        if (!response.data.error) {
          const updatedUser = {
            ...user,
            role: disableorenable === "disable" ? "disable" : "user",
          };
          setUser(updatedUser);
        }
      })
