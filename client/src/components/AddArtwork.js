...
function AddArtworkComponent({ isAdd, artwork_id }) {
    ...
    useEffect(() => {
        ...
        if (!isAdd) {
            Axios.get(`${deploy_api_url}/api/artworks/${artwork_id}`).then((response) => {
        ...
        // get all the tags
        Axios.get(`${deploy_api_url}/api/tags`).then((response) => {
                    ...
                    .then((fileNames) => {
                        ...
                        Axios.post(`${deploy_api_url}/api/artworks`, {
                            ...
                            tagArray.forEach((tag) => {
                                Axios.patch(`${deploy_api_url}/api/tags/updateTagCountIncrease/${tag}`, {
                                    increseBy: 1,
                                }).then((response) => {
            ...
            .then(() => {
                ...
                Promise.all(uploadPromises)
                    .then((fileNames) => {
                        ...
                        Axios.patch(`${deploy_api_url}/api/artworks/mainInfo/${artworkToUpdate._id}`, {
                            title: title,
                            description: description,
                            price: parseFloat(price),
                        ...
                        if (decreaseTagArray.length > 0) {
                            decreaseTagArray.forEach((tag) => {
                                Axios.patch(
                                    `${deploy_api_url}/api/tags/updateTagCountDecrease/${tag}`,
                                    {
                                        decreaseBy: 1,
                                    }
                                )
                        ...
                        if (increaseTagArray.length > 0) {
                            increaseTagArray.forEach((tag) => {
                                Axios.patch(
                                    `${deploy_api_url}/api/tags/updateTagCountIncrease/${tag}`,
                                    {
                                        increaseBy: 1,
                                    }
                                )
