...
function CartComponent({ buyerId }) {
  ...
  useEffect(() => {
    Axios.get(`${deploy_api_url}/api/purchases/unpaid/buyer`, {
      ...
      .then((response) => {
        ...
        const promises = purchaseList.map((purchase) => {
          return Axios.get(
            `${deploy_api_url}/api/artworks/${purchase.artwork_id}`
          )
        ...
        Promise.all(promises).then((updatedPurchases) => {
  ...
  const handleDelete = () => {
    ...
    if (purchaseId) {
      Axios.delete(`${deploy_api_url}/api/purchases/${purchaseId}`)
        .then((response) => {
  ...
  return (
    <div id="cart-container" className="cart m-5 p-5 capitalize">
      <div className="text-3xl font-bold subpixel-antialiased p-3 m-5">
        shopping cart
      </div>
      <Modal
        isOpen={isModalOpen}
      ...
      {isLoading && (
        <div>
          <div className="text-4xl font-medium subpixel-antialiased p-5 m-5 text-center">
            Loading...
          </div>
        </div>
      )}
          ...
          {purchases.map((purchase, index) => (
            <div key={index} className="cartItems p-3 m-5 flex flex-wrap">
              <div className="imageBox w-full md:w-1/2 lg:w-1/4 flex flex-col items-center justify-center border-2">
                {purchase.artwork && (
                  <img
                    className="w-90 h-60 m-auto my-3"
                    src={purchase.artwork.cover_url}
                    alt="artwork cover"
                  ></img>
                )}
              </div>
              {purchase.artwork && (
                <div className="generalInfo w-full md:w-1/2 lg:w-1/4 flex flex-col items-center justify-center font-bold">
              ...
              <div className="price w-full md:w-1/2 lg:w-1/4 flex flex-col items-center justify-center">
                ...
                {purchase.artwork && (
                  <button
                    className="w-1/2 capitalize bg-blue-400 text-white px-4 py-2 rounded-lg hover:bg-blue-200 p-1 m-1"
                ...
                <button
                  className="w-1/2 capitalize bg-red-400 text-white px-4 py-2 rounded-lg hover:bg-red-200 p-1 m-1"
                  onClick={() => {
