...
function AddArtworkComponent({ isAdd, artwork_id }) {
  ...
  useEffect(() => {
    ...
    if (!isAdd) {
      Axios.get(`${deploy_api_url}/api/artworks/${artwork_id}`)
        .then((response) => {
  ...
  const [showCreateSuccessfullyModal, setShowCreateSuccessfullyModal] =
    useState(false);
      ...
      .then(() => {
        ...
        Promise.all(uploadPromises)
          .then((fileNames) => {
            ...
            Axios.patch(
              `${deploy_api_url}/api/artworks/mainInfo/${artworkToUpdate._id}`,
              {
                title: title,
                description: description,
                price: parseFloat(price),
                ...
                <div className="mb-4">
                  ...
                  <input
                    ...
                    defaultValue={tagArray
                      .map((id) => {
                        const matchingTag = tagList.find(
                          (tag) => tag._id === id
                        );
